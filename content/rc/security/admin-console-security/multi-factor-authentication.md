...
When you log in to the Redis Enterprise Cloud Admin Console, you can select  either an authentication code sent by text message or an authentication code shown in your authenticator app.
...
1. Add Redis Enterprise Cloud to the app:
    1. From the **User Profile** view in your Redis Enterprise Cloud account, click **Multi-Factor Authentication**.
...
After MFA is enforced for the account, all users who do not have MFA enabled will be required to configure MFA the next time they log in to the Redis Enterprise Cloud Admin Console.
...
{{< tip >}}
We recommend that you notify all of your Redis Enterprise Cloud Admin Console users before enabling MFA enforcement.
{{< /tip >}}
